...
function buildBullPrompt(dataSection) {
...
function buildBearPrompt(dataSection) {
...
function buildJudgePrompt(dataSection, bullCase, bearCase) {
...
function buildCouncilFinalPrompt(dataSection, bullCase, bearCase, judge) {
...
async function runCouncil(apiKey, modelId, snapshot, signal, onProgress) {
  ...
  const [bullCase, bearCase] = await Promise.all([
    callAgentText(apiKey, modelId, buildBullPrompt(dataSection), signal),
    callAgentText(apiKey, modelId, buildBearPrompt(dataSection), signal),
  ]);

  // ── Phase 2: Judge menimbang ──────────────────────────────────────────────
  onProgress?.('judge');
  const judge = await callAgentStructured(
    apiKey, modelId, buildJudgePrompt(dataSection, bullCase, bearCase), JUDGE_SCHEMA, signal
  );

  // ── Phase 3: Portfolio Manager final decision ────────────────────────────
  onProgress?.('final');
  const finalRes = await callGemini(
    apiKey, modelId, buildCouncilFinalPrompt(dataSection, bullCase, bearCase, judge), signal, false
  );
...
function viewFlowCard(snap, analysis) {
  ...
  if (cvd) {
    ...
    cards.push(richCard({
      ...
      extra: `beli ${cvd.buyVol?.toFixed(0)} vs jual ${cvd.sellVol?.toFixed(0)} BTC`,
