...
Penambahan layer bandarmologi terkuat: **aliran institusi & lintas-bursa**.

### Sumber baru (gratis, no key)
- **Stablecoin supply (DefiLlama)** — "dry powder" likuiditas. Supply naik = amunisi beli menunggu (bullish lingkungan).
- **Bitcoin ETF flows (DefiLlama, best-effort)** — aliran institusi langsung. Net inflow = akumulasi institusi (sinyal bullish terkuat).
- **CVD / Cumulative Volume Delta (Binance aggTrades)** — order flow riil: agresor beli vs jual dari 1000 trade terakhir.
- **Spot-Perp basis (Binance, computed)** — perp premium = long leverage crowded (risiko squeeze); discount = bearish/hedging.
- **Multi-exchange funding (Bybit + OKX)** — bandingkan funding lintas bursa untuk lihat crowding lebih luas.

### Liquidation (alternatif GRATIS pengganti CoinGlass)
- **Liquidation Magnet Levels (computed, NO KEY)** — estimasi level di mana posisi leverage terlikuidasi (25x/50x/100x), zona magnet atas (short liq → squeeze naik) & bawah (long liq → cascade turun). Selalu muncul, dihitung dari harga current. Inilah inti actionable dari liquidation heatmap, tanpa perlu API berbayar.
- **Coinalyze (GRATIS, perlu free key)** — pengganti langsung CoinGlass. Liquidation history + OI agregat lintas bursa. Daftar gratis di coinalyze.net (40 call/menit), paste key di Settings. CoinGlass TERNYATA BERBAYAR, jadi diganti Coinalyze yang benar-benar gratis.

### Kaidah AI baru (#12)
- ETF inflow + CVD + smart money L/S searah = confluence bandarmologi terkuat → boleh HIGH confidence
- PERP_PREMIUM tinggi = waspada long squeeze
- Liquidation bias: LONGS_REKT bisa jadi bottom lokal, SHORTS_REKT bisa jadi top lokal

### Field output baru
`flowView` — interpretasi AI atas ETF/stablecoin/CVD/basis/liquidation.

### UI
Kartu baru "Institutional & Cross-Exchange Flow" (border ungu) dengan 6 mini-card: ETF flow, stablecoin, CVD, basis, liquidation, multi-funding.

### Catatan
- CoinGlass key dikirim dari browser ke Edge function via header `x-coinglass-key` (server-side fetch, tidak kena CORS). Key tetap tersimpan lokal di browser.
- ETF flows DefiLlama pakai endpoint best-effort; kalau tidak tersedia, otomatis masuk "sumber opsional" tanpa mengganggu analisis. ETF data yang reliable bisa datang lewat CoinGlass.
